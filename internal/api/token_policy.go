...
// tokenPolicyRequest is the JSON body for creating a token policy.
...
type tokenPolicyRequest struct {
...
// tokenPolicyUpdateRequest is the JSON body for updating a token policy.
// Only perm_write is mutable after creation.
type tokenPolicyUpdateRequest struct {
	PermWrite bool `json:"perm_write"`
}
...
// Pass nil for domain, subname, and/or rrtype to create the default policy.
func (c *Client) CreateTokenPolicy(ctx context.Context, tokenID string, domain, subname, rrtype *string, permWrite bool) (*TokenPolicy, error) {
	body := tokenPolicyRequest{
		Domain:    domain,
		Subname:   subname,
		Type:      rrtype,
		PermWrite: permWrite,
	...
	resp, err := c.do(ctx, http.MethodPost, tokenPoliciesPath(tokenID), body)
	if err != nil {
...
func (c *Client) UpdateTokenPolicy(ctx context.Context, tokenID, policyID string, permWrite bool) (*TokenPolicy, error) {
	body := tokenPolicyUpdateRequest{PermWrite: permWrite}

	resp, err := c.do(ctx, http.MethodPatch, tokenPolicyPath(tokenID, policyID), body)
	if err != nil {
